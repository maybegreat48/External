...
namespace ext
{
	...
	public:
		...
		void apply_features()
		{
			g_script_loader->set_feature(program::features::self_godmode, this->self.godmode);
			g_script_loader->set_feature(program::features::self_fast_run, this->self.super_run);
			g_script_loader->set_feature(program::features::self_no_cops, this->self.never_wanted);
			g_script_loader->set_feature(program::features::self_invisible, this->self.invisibility);

			g_script_loader->set_feature(program::features::weapons_infinite_ammo, this->weapons.infinite_ammo);
			g_script_loader->set_feature(program::features::weapons_infinite_clip, this->weapons.infinite_mag);

			g_script_loader->set_feature(program::features::vehicle_godmode, this->vehicle.god_mode);
			g_script_loader->set_feature(program::features::vehicle_targettable, this->vehicle.is_targetable);
			g_script_loader->set_feature(program::features::vehicle_horn_boost, this->vehicle.horn_boost);

			g_script_loader->set_feature(program::features::mobile_off_radar, this->self.off_radar);

			g_script_loader->set_feature(program::features::spawn_in_vehicle, this->spawn.spawn_inside);
		}
